...
def Generate_DE_source(sample_size, alpha, random_seed=None):

    if random_seed is not None:
        np.random.seed(random_seed)

    # Generate A and B from N(0,1)
    A = np.random.normal(0, 1, sample_size)
    B = np.random.normal(0, 1, sample_size)
    C = np.random.normal(0, 1, sample_size)

    # Generate M based on P(M=1 | A > 0.5 and B > 0.5) = alpha, otherwise 0
    M = np.zeros(sample_size)
    M[(A > 0.5) & (B > 0.5)] = np.random.binomial(1, 1- alpha, np.sum((A > 0.5) & (B > 0.5)))


    X = np.random.binomial(1, 0.5, sample_size)

    # Generate Y based on P(Y=1 | X=1 and B > 0.5) = alpha, otherwise 0

    logit_Y = -1 + 3*C
    pr_Y = np.clip(1 / (1 + np.exp(-logit_Y)), 1e-6, 1 - 1e-6)
    Y = np.random.binomial(1, pr_Y, sample_size)
    print(sum(Y))
    Y[(X == 1) & (B > 0.5)] = np.random.binomial(1, alpha, np.sum((X == 1) & (B > 0.5)))

    X_exp = np.column_stack((X, M, C))

    return X_exp, Y

def Generate_Do_target(sample_size, alpha, random_seed=None):
    if random_seed is not None:
        np.random.seed(random_seed)
    ...
    # Generate M based on P(M=1 | A > 0.5 and B > 0.5) = alpha, otherwise 0
    M = np.zeros(sample_size)
    M[(A > 0.5) & (B > 0.5)] = np.random.binomial(1, alpha, np.sum((A > 0.5) & (B > 0.5)))

    df = pd.DataFrame({
        "U1": A,
        "U2": B,
        "Z": M  # Example Z distribution
    })

    # Filter where U1 > 0.5 and U2 > 0.5
    filtered_df = df[(df["U1"] > 0.5) & (df["U2"] > 0.5)]

    # Compute P(Z=1 | U1 > 0.5, U2 > 0.5)
    p_conditional = filtered_df["Z"].mean()

    print(f"P(Z=1 | U1>0.5, U2>0.5) = {p_conditional:.4f}")

    # Generate X based on P(X=1 | A > 0.5) = alpha, otherwise 0
    X = np.zeros(sample_size)
    X[A > 0.5] = np.random.binomial(1, alpha, np.sum(A > 0.5))
    ...
    X_obs = np.column_stack((X, M, C))

    return X_obs, Y

def Generate_De_target(sample_size, alpha, random_seed=None):
    if random_seed is not None:
        np.random.seed(random_seed)
...
for Ne in ne_list:
    for alpha in alpha_list:
        ...
        for k in range(n_runs):
            ...
            """Let's assume that U is a latent and that MB(Y)=(T,Z)"""
            P_Hzc = {}
            P_Hz_not_c = {}
            P_HZ = {}
            P_HZC = {}
            ...
            print('Set with the max P_Hz', max_P_HZ)

            df.loc[len(df)] = new_row
            list_of_Hz_0.append(P_HZ[(0, 2)])
            list_of_Hz_01.append(P_HZ[(0, 1, 2)])
        ...
        print('Log loss in ALL', Log_loss_all)


        print(df)
        print("PHz(X, )", list_of_Hz_0)
        print("PHz(X, Z)", list_of_Hz_01)

        # Create a DataFrame
        AUC_results = pd.DataFrame({
            'PHz(X, )': list_of_Hz_0,
            'PHz(X, Z)': list_of_Hz_01,
        ...
        AUC_results.to_csv(f'AUC_m_bias_Ne{Ne}_alpha{alpha}.csv', index=False)
        Exp_results.to_csv(f'experiments_m_bias_Ne{Ne}_alpha{alpha}.csv', index=False)
