...
def Generate_DE_source(sample_size, intercept_Y, b_T_Y, b_Z2_Y,b_Z1_Y,Z1_mean_P,Z1_std_P,Z2_mean_P,Z2_std_P, random_seed=None):
    ...
    H = np.random.normal(0, 3, sample_size)


    # Generate Y based on the logistic model
    logit_Y = intercept_Y + b_T_Y * T + b_Z2_Y * Z2 - 2*H + b_Z1_Y * Z1 + e
...
def Generate_Do_target(sample_size, intercept_Y, b_T_Y, b_Z2_Y, b_Z1_Y, intercept_T, b_Z2_T, Z1_mean_PT,Z1_std_PT,Z2_mean_PT,Z2_std_PT, random_seed=None):
    ...
    logit_T = intercept_T + b_Z2_T * Z2 + 2*H
...
def score_func(reg_variables):
    ...
    P_HZ[reg_variables] = math.exp(P_Hzc[reg_variables]) / (math.exp(P_Hzc[reg_variables]) + math.exp(P_Hz_not_c[reg_variables]))
...
for Ne in Ne_list:
    ...
    for k in range(n_runs):
        ...
        all_results.append(AUC_results)
        ...
        df_AUC_results = pd.DataFrame(all_results)
        df_AUC_results.to_csv(f'AUC_Fig1b_N{Ne}.csv', index=False)
        Exp_results.to_csv(f'experiments_Fig1b_results_N{Ne}.csv', index=False)
